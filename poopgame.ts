...
// =================== 상태 정의 ===================
export const STATE_INIT = 3000;
export const STATE_READY = 3001;
export const STATE_PLAYING = 3002;
export const STATE_JUDGE = 3004;
export const STATE_END = 3005;
export const STATE_INTRO = 3006;
...
// =================== 게임 상태 전환 ===================
export function startState(state: number) {
  _state = state;
  _stateTimer = 0;
      ...
      if (_flushTime >= 3) {
        ...
        for (let i = 0; i < poopkeys.length; i++) {
          ...
          if (ZepMap.getObjectWithKey(key).tileY == ZepMap.height - 1) {
            ZepMap.putObjectWithKey(ZepMap.getObjectWithKey(key).tileX, ZepMap.height - 1, null, { key });
            poopkeys.splice(i--, 1);
          }
        }
      }
